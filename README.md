...
> 秋招信息聚合与投递追踪工具——从"到处开表格记进度"到一个真正能用的全栈小应用。

个人秋招/校招场景下的信息管理工具：维护一份公司库（类型、规模、是否开放招聘、内推/官网链接），追踪每一次投递的岗位、base 城市和进展阶段，并把简历拆分成可复制粘贴的模块，方便针对不同公司快速填表投递。单用户本地工具，也可以部署到云端或打包成 Windows 可执行文件。

**项目亮点：**
- **全栈实现**：FastAPI + SQLAlchemy + SQLite 后端，Jinja2 + HTMX 前端——不依赖 React/Vue，局部刷新体验接近 SPA
- **自定义设计系统**：玻璃拟态卡片、渐变强调色、滚动 reveal / 数字滚动等微交互，深浅两套主题都做过
- **完整工程化**：pytest 测试覆盖、session 登录鉴权、Docker + Fly.io 云部署配置、GitHub Actions 自动打包 Windows 可执行文件
- **AI 辅助全栈开发**：全程使用 Claude Code 协作完成，从需求梳理、技术方案设计到测试驱动开发、代码评审
